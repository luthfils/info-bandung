...
if(isset($_POST['submit'])){
	...
	$contoh=addslashes(htmlentities(ucwords($_POST['contoh'])));
	$id_jenis=addslashes(htmlentities($_POST['id_jenis']));

	$nama_gambar=$_FILES['gambar']['name'];
	$type = $_FILES['gambar']['type'];
	$ukuran=$_FILES['gambar']['size'];
	
	if(empty($contoh) || empty($id_jenis) || empty($nama_gambar)){
		echo "Maaf, form belum lengkap!!</br></br> <a href=index.php><input type='button' class='formstyletwo-warning' value='Silahkan ulangi'></a>";
	}else{  
		$query_contoh=mysql_query("select * from t_contoh where contoh='$contoh'");
		$cek=mysql_num_rows($query_contoh);
		if($cek>0){
			echo "Maaf, contoh informasi sudah dipakai!!</br></br> <a href=index.php><input type='button' class='formstyletwo-warning' value='Silahkan ulangi'></a>";
		}else{
			if($type != "image/gif"  &&  $type != "image/jpg"  && $type != "image/jpeg" && $type != "image/png") {
				...
				if($ukuran>1000000){
					...
					$uploaddir='../../gambar/info_bandung/gambar_contoh/';
					$rnd=date(His);				
					...
					{
						$query=mysql_query("insert into t_contoh(id_jenis,contoh,gambar) 
									values('$id_jenis','$contoh','$nama_file_upload')");
							
						if($query){
							echo "Data Anda berhasil disimpan </br></br> <a href=view_data.php> <input type='button' class='formstyletwo-warning' value='View Contoh Informasi'></a>";
						}else{
							echo mysql_query();
						}
					}else{
