...
if(isset($_POST['submit'])){
	...
	if(empty($kategori) ||  empty($nama_gambar)){
		echo "Maaf, form belum lengkap!! </br></br> <a href=index.php><input type='button' class='formstyletwo-warning' value='Silahkan ulangi'></a>";
	}else{  
		$query_kategori=mysql_query("select * from t_kategori where kategori='$kategori'");
		$cek=mysql_num_rows($query_kategori);
		if($cek>0){
			echo "Maaf, nama kategori informasi sudah dipakai!! </br></br> <a href=index.php><input type='button' class='formstyletwo-warning' value='Silahkan ulangi'></a>";
		}else{
			if($type != "image/gif"  &&  $type != "image/jpg"  && $type != "image/jpeg" && $type != "image/png") {
				echo "File harus jpg,jpeg,png,gif!! <a href=index.php><input type='button' class='formstyletwo-warning' value='Silahkan ulangi'></a>";
			}else{
				if($ukuran>1000000){
					...
					{
						$query=mysql_query("insert into t_kategori(kategori,gambar) 
									values('$kategori','$nama_file_upload')");
