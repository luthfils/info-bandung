...
if(isset($_POST['submit'])){
	...
	if(empty($judul) || empty($id_informasi) || empty($deskripsi) || empty($nama_gambar)){
		echo "Maaf, form belum lengkap!! </br></br> <a href=index.php><input type='button' class='formstyletwo-warning' value='Silahkan ulangi'></a>";
	}else{  
		$query_judul=mysql_query("select * from t_gambar where judul='$judul'");
		$cek=mysql_num_rows($query_judul);
		if($cek>0){
			echo "Maaf, judul gambar sudah dipakai!! </br></br> <a href=index.php><input type='button' class='formstyletwo-warning' value='Silahkan ulangi'></a>";
		}else{
			if($type != "image/gif"  &&  $type != "image/jpg"  && $type != "image/jpeg" && $type != "image/png") {
				...
				if($ukuran>1000000){
					echo "File Yang Di izinkan Hanya berukuran kurang dari 1MB!! </br></br> <a href=index.php><input type='button' class='formstyletwo-warning' value='Silahkan ulangi'></a>";
				}else{
					...
					{
						$query=mysql_query("insert into t_gambar(id_informasi,judul,gambar,deskripsi) 
									values('$id_informasi','$judul','$nama_file_upload','$deskripsi')");
