...
if(isset($_POST['submit'])){
	...
	if(empty($nama_gambar_socmed) ||  empty($nama_gambar)){
		echo "Maaf, form belum lengkap!!</br></br> <a href=index.php><input type='button' class='formstyletwo-warning' value='Silahkan ulangi'></a>";
	}else{  
		$query_nama_gambar_socmed=mysql_query("select * from t_gambar_socmed where nama_gambar_socmed='$nama_gambar_socmed'");
		$cek=mysql_num_rows($query_nama_gambar_socmed);
		if($cek>0){
			echo "Maaf, nama gambar social media sudah dipakai!! </br></br> <a href=index.php> <input type='button' class='formstyletwo-warning' value='Silahkan ulangi'></a>";
		}else{
			if($type != "image/gif"  &&  $type != "image/jpg"  && $type != "image/jpeg" && $type != "image/png") {
				...
				if($ukuran>1000000){
					...
					{
						$query=mysql_query("insert into t_gambar_socmed(nama_gambar_socmed,gambar) 
									values('$nama_gambar_socmed','$nama_file_upload')");
