...
<html>
	<head>
		...
		<?php
			...
			$query = "SELECT * FROM t_jenis WHERE id_jenis = '$id_jenis' ";
			$proses = mysql_query($query);
			$r = mysql_fetch_array($proses);
		?>
		<title><?php echo $r['jenis']; ?></title>
...
<body>
	...
	<div data-role="content" data-theme="a">
		<ul data-icon="arrow-r" data-role="listview" data-filter="true" data-filter-placeholder="Pencarian..." data-inset="false">	
			<?php
				$id_jenis = $_GET['id_jenis'];
				$query = "SELECT * FROM t_contoh WHERE id_jenis= '$id_jenis' ORDER BY id_contoh asc";
				$proses = mysql_query($query);	
				while($data=mysql_fetch_array($proses)){
			?>
			<li>
				<a href="daftar.php?id_contoh=<?php echo $data['id_contoh'] ?>">
				<?php echo "<img src='gambar/info_bandung/gambar_contoh/$data[gambar]' height='96%'>" ?>
				<p class="batas"><?php echo $data['contoh'] ?></p>
				</a>
			</li>
			<?php } ?>
		</ul>
